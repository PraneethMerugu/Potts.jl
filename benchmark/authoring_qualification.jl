VERSION == v"1.12.6" ||
    error("authoring qualification requires Julia 1.12.6; found $VERSION")
...
struct BenchmarkIncrement <: AbstractProcess end

ports(::BenchmarkIncrement) = (
    InputPort(Int, :state),
...
function semantic_model()
    compose(:AuthoringBenchmark; scale=SCALE) do model
        state = store!(
            model,
            :state,
...
all(values(checks)) ||
    error("authoring performance budget failed: $(checks)")
...
println("AUTHORING_QUALIFICATION=", result)
