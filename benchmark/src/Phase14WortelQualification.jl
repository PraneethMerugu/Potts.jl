const PHASE14_WORTEL_EVIDENCE_SCHEMA = "1.0.0"

@kernel function _phase14_activity_energy_probe!(
...
function _phase14_wortel_fixture(dims::NTuple{2, Int})
    spacing = (1.0f0, 1.0f0)
    ...
    cell > 0 || error("Phase 14 qualification fixture contains no finite cells")
...
function _phase14_activity_program(
...
function _phase14_build_wortel(name::String, dims::NTuple{2, Int};
        seed::UInt64, observation_cadence::Int,
        require_gpu_native::Bool = true)
    fixture = _phase14_wortel_fixture(dims)
    adaptor = _backend_adaptor(name)
    state = Adapt.adapt(adaptor, fixture.compiled)
    scientific_storage_valid(state) ||
        error("$name Phase 14 scientific state is not one backend-resident array tree")
    ...
    program = _phase14_activity_program(
...
function _phase14_actionable_proposal(fixture)
    ...
    error("Phase 14 qualification fixture has no finite-cell copy proposal")
end

function _phase14_energy_probe!(run)
    proposal = _phase14_actionable_proposal(run.fixture)
...
function _phase14_unobserved_and_observed_qualification!(run)
    energy = _phase14_energy_probe!(run)
    SciMLBase.step!(run.coupled)
    report = current_mcs_report(run.coupled.potts)
    report.accepted_copies > 0 ||
        error("Phase 14 production trace exercised no accepted copies")
    report.acceptance_rejections > 0 ||
        error("Phase 14 production trace exercised no acceptance rejection")
    report.same_owner_no_ops > 0 ||
        error("Phase 14 production trace exercised no same-owner no-op")
    ...
    record.observation === :activity_summary ||
        error("Phase 14 activity observation did not publish its declared record")
...
function qualify_phase14_wortel_backend(
        name::String; profile::String = "paper")
    name in ("metal", "amdgpu") || throw(ArgumentError(
        "Phase 14 Wortel G2 requires --backend=metal or --backend=amdgpu"))
    profile in ("smoke", "paper") || throw(ArgumentError(
        "Phase 14 Wortel profile must be smoke or paper"))
    _, device = load_backend(name)
    extent = profile == "paper" ? 128 : 48
    dims = (extent, extent)
    main = _phase14_build_wortel(
        name, dims; seed = UInt64(0x7068617365313401),
        observation_cadence = 5)
    execution = _phase14_unobserved_and_observed_qualification!(main)
    replay_dims = (min(extent, 64), min(extent, 64))
    continuation = _phase14_replay_restart_qualification(
        name, replay_dims)
...
function phase14_wortel_result(
        name::String, qualification::Dict)
    return Dict(
        "schema_version" => PHASE14_WORTEL_EVIDENCE_SCHEMA,
...
function write_phase14_wortel_result(result::Dict)
    ...
    mkpath(directory)
    path = joinpath(
        directory, "$(timestamp)-phase14-wortel-gpu-native-$(profile).toml")
    open(path, "w") do io
