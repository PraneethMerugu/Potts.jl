VERSION == v"1.12.6" ||
    error("native-field qualification requires Julia 1.12.6; found $VERSION")
...
const output_path = option(
    ...
    joinpath(
        @__DIR__,
        "results",
        "native-field-$(backend_name).toml",
    ),
)
...
println("NATIVE_FIELD_RESULT=", output_path)
println("NATIVE_FIELD_QUALIFICATION=", result)
