VERSION == v"1.12.6" ||
    error("Wortel model Metal profiling requires Julia 1.12.6; found $VERSION")
...
directory = joinpath(
    PottsBenchmarks.RESULTS_ROOT, "wortel-device-code", "metal")
mkpath(directory)
...
filesize(path) > 0 ||
    error("Wortel model Metal device-code capture was empty")
println("WORTEL_MODEL_METAL_DEVICE_CODE=", path)
