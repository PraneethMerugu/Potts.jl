...
if !isdefined(Main, :LifecycleOperationFixtures)
    include("../../test/fixtures/LifecycleOperationFixtures.jl")
end
...
function _direct_lifecycle_runtimes(completed, initial, device_array; seed)
    ...
    reference_executable = Potts._lower_execution_plan(
        scheduled,
        SequentialCPM(),
        CPUBackend(),
        Float32,
    )
    device_executable = Potts._lower_execution_plan(
        scheduled,
        CheckerboardSweepCPM(),
        CPUBackend(),
        Float32,
    )
    reference_initial = Potts._core_initial_state(
        reference_executable, initial, UInt64(seed), UInt32(1)
    )
    device_initial = Potts._core_initial_state(
    ...
    workspace = device_array === Array ? host_candidate.engine_workspace :
        CorePotts.adapt_program_runtime(
            device_array, host_candidate
        ).engine_workspace
...
function run_lifecycle_retirement_execution(
    ...
    for state in (reference, host_candidate_state)
        ...
        source = CorePotts.CompilerSPI.tracker_source_view(
...
function run_forbid_extinction_execution(
    ...
    for seed in UInt64(1):UInt64(64)
        ...
        if _test_checkerboard_core(candidate).report[4] > 0
            selected_seed = seed
            reference = candidate
            break
        end
    ...
    workspace = CorePotts.adapt_checkerboard_workspace(
        device_array, _test_checkerboard_core(candidate)
    )
    ...
    reference_core = _test_checkerboard_core(reference)
    @test report == reference_core.report
    @test report[4] > 0
    @test to_host(workspace.state.ownership) ==
        reference_core.state.ownership
    @test count(==(Int32(1)), reference_core.state.ownership) == 1
    @test CorePotts.Adapt.adapt(Array, workspace.state.trackers).values ==
        reference_core.state.trackers.values
    ...
    queued_workspace = CorePotts.adapt_checkerboard_workspace(
        device_array, _test_checkerboard_core(queued_candidate)
    )
...
function run_lifecycle_resolution_policy_execution(
    ...
    _, _, reject_executable, workspace = _direct_lifecycle_runtimes(
        reject_completed, reject_initial, device_array; seed = 0x87
    )
    initial_ownership = copy(to_host(workspace.state.ownership))
