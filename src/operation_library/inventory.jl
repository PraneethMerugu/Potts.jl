...
function _builtin_operation_declarations()
    return (
        ...
        (occupancy, 2), (distance, 2),
        (contact_edge_count, 2), (boundary_site_count, 2),
        (neighbor_cell_count, 2), (contact_measure, 3),
        (neighbor_property_sum, 3), (neighbor_property_mean, 4),
        (global_interface_measure, 3),
...
function _operation_inventory(graph::NormalizedTermGraph)
    return NamedTuple[ (
        ...
        source_requirements = schema.transfer.source_requirements,
...
function _builtin_operation_inventory()
    ...
    for (operation, arity) in _builtin_operation_declarations()
        ...
        push!(rows, (
            ...
            source_requirements = transfer.source_requirements,
            cpu = transfer.cpu,
            gpu = transfer.gpu,
