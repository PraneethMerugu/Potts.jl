...
function RegisteredStatement(id::Union{Symbol, StatementID}, schema::Symbol,
    ...
    return RegisteredStatement(_statement_core(
        id,
        (; schema, version, arguments = _defensive_tuple(arguments)),
        (; kwargs...),
        source,
    ))
end
...
function _map_symbolic_payload(f, value)
    symbolic = !(Symbolics.symbolic_type(value) isa
...
function register_statement(registry::StatementRegistry, schema::Symbol,
        version::VersionNumber, implementation_identity::AbstractString)
    key = (schema, version)
    definition = StatementDefinition(schema, version, String(implementation_identity))
