...
function _lower_cartesian_domain(ir::AnalyzedTermIR)
    ...
    lowered = _lower_cartesian_domain(
        ir.source, domain, shape, declarations, kinds
    )
    report = CorePotts.CompilerSPI.cartesian_domain_report(lowered.domain)
    periodic = ntuple(axis ->
        report.faces[2 * axis - 1].kind === CorePotts.CompilerSPI.PeriodicCartesianFace,
        length(shape),
    )
    return (; lowered..., domain_record = domain, shape,
        dimensions = length(shape), periodic, declarations, kinds,
        count = length(kinds))
end
