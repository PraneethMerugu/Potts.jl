...
function _lower_relationships(
    ...
    return Tuple(
        begin
                ...
                capacity isa Real && isinteger(capacity) || throw(
                    ArgumentError(
...
function _lower_observations(
    ...
    for (record_index, record) in enumerate(records)
        ...
        if node.operation === :occupancy && length(node.operands) == 2
            ...
            kind_node.payload isa KindBindingPayload || throw(
                ArgumentError(
                    "observation `$name` has no resolved kind binding"
                )
            )
            haskey(kinds, kind_node.payload.identity) || throw(
                ArgumentError(
        ...
        elseif node.operation === :degree &&
            ...
            relationship_index === nothing && throw(
                ArgumentError(
            ...
            endpoint_node.payload isa LiteralPayload || throw(
                ArgumentError(
...
function _append_exclusive_access!(
    ...
    policy isa CorePotts.CompilerSPI.ExclusiveWriteAccess || throw(
        ArgumentError(
...
function _lower_core_program(
    ...
    CorePotts.CompilerSPI.cartesian_domain_report(cartesian_domain).shape == shape ||
        error("lowered Cartesian domain shape differs from its source declaration")
    count = length(kinds)
    ...
    program_fingerprint = _sha256_hex(
        ...
        T,
        shape,
        CorePotts.CompilerSPI.cartesian_domain_report(cartesian_domain),
        proposal_offsets,
        contact_offsets,
