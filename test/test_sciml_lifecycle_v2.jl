const G5H2V2_SII = SymbolicIndexingInterface

@parameters begin
    g5h2v2_target = 4.0
    g5h2v2_strength = 2.0
    g5h2v2_temperature = 3.0
end
...
function _g5h2v2_fixture(
    ...
    source = PottsSystem(
        ...
        statements = StatementSet((
            ...
            Volume(
                cell;
                target = g5h2v2_target,
                strength = g5h2v2_strength,
            ),
        ...
        unknowns = [g5h2v2_marker],
        parameters = [
            g5h2v2_target,
            g5h2v2_strength,
            g5h2v2_temperature,
        ],
        initial_conditions = Dict(g5h2v2_marker => 0.25),
    )
...
function _g5h2v2_same_state(left, right)
    return left.mcs == right.mcs &&
           ...
           left[:g5h2v2_marker] == right[:g5h2v2_marker] &&
           left[:g5h2v2_marker_snapshot] ==
...
@testset "late runtime profiles and SciML entry points" begin
    ...
    @test eltype(G5H2V2_SII.parameter_values(default_integrator)) === Float32
    @test eltype(G5H2V2_SII.parameter_values(sequential_integrator)) === Float64
    @test G5H2V2_SII.getp(problem, g5h2v2_target)(problem) == 5.0
...
@testset "complete remake dimensions" begin
    ...
    remade = remake(
        problem;
        u0 = Dict(g5h2v2_marker => replacement),
        p = (
            g5h2v2_target => 6.5,
            g5h2v2_strength => 2.25,
            g5h2v2_temperature => 1.75,
        ),
        tspan = (2, 5),
        seed = 0xbeef,
        replica = 4,
    ...
    @test G5H2V2_SII.getp(remade, g5h2v2_target)(remade) == 6.5
    @test G5H2V2_SII.getp(problem, g5h2v2_target)(problem) == 5.0

    integrator = init(remade; scalar_type = Float32, save_start = false)
    @test integrator.t == 2
    @test all(==(7.0f0), integrator.u[:g5h2v2_marker])
    @test G5H2V2_SII.getp(integrator, g5h2v2_target)(integrator) == 6.5f0
    @test all(==(1.25), G5H2V2_SII.state_values(problem, 1))
...
@testset "SymbolicIndexingInterface contract" begin
    ...
    integrator = init(
        ...
        scalar_type = Float32,
        save_start = false,
        observables = (:g5h2v2_marker_snapshot,),
    )

    @test G5H2V2_SII.symbolic_container(system) === system
    @test G5H2V2_SII.symbolic_container(problem) === system
    @test G5H2V2_SII.symbolic_container(integrator) === system
    @test G5H2V2_SII.is_parameter(system, g5h2v2_target)
    @test G5H2V2_SII.parameter_index(system, g5h2v2_target) == 1
    @test any(isequal(g5h2v2_target), G5H2V2_SII.parameter_symbols(system))
    @test G5H2V2_SII.is_variable(system, g5h2v2_marker)
    @test G5H2V2_SII.variable_index(system, g5h2v2_marker) == 1
    @test any(isequal(g5h2v2_marker), G5H2V2_SII.variable_symbols(system))
    @test G5H2V2_SII.is_observed(system, :g5h2v2_marker_snapshot)
    @test :g5h2v2_marker_snapshot in
          G5H2V2_SII.all_variable_symbols(system)
    @test :mcs in G5H2V2_SII.independent_variable_symbols(system)
    @test any(isequal(g5h2v2_target), G5H2V2_SII.all_symbols(system))
    ...
    state_index = G5H2V2_SII.variable_index(system, g5h2v2_marker)
    @test hasmethod(
        G5H2V2_SII.state_values,
    ...
    target_getter = G5H2V2_SII.getp(system, g5h2v2_target)
    marker_getter = G5H2V2_SII.getsym(system, g5h2v2_marker)
    ...
    target_setter = G5H2V2_SII.setp(integrator, g5h2v2_target)
    target_setter(integrator, 6.0)
    @test target_getter(integrator) == 6.0f0
    transaction = G5H2V2_SII.setp(
        integrator,
        (g5h2v2_target, g5h2v2_strength),
    )
    @test_throws ArgumentError transaction(integrator, (7.0, Inf))
    @test target_getter(integrator) == 6.0f0
    @test G5H2V2_SII.getp(system, g5h2v2_strength)(integrator) == 1.5f0
    transaction(integrator, (7.0, 2.0))
    @test target_getter(integrator) == 7.0f0
    @test G5H2V2_SII.getp(system, g5h2v2_strength)(integrator) == 2.0f0

    immutable_setter = G5H2V2_SII.setp(problem, g5h2v2_target)
    ...
    observation_getter = G5H2V2_SII.getsym(
        system,
        :g5h2v2_marker_snapshot,
    )
    @test observation_getter(solution, 1) ==
          solution.u[1][:g5h2v2_marker_snapshot]
...
@testset "saving, callbacks, and termination" begin
    ...
    @test endpoints(4; idxs = :g5h2v2_marker) ==
          endpoints.u[end][:g5h2v2_marker]
...
@testset "checkpoint identity and exact continuation" begin
    ...
    resumed_integrator = init(
        remake(problem; p = (
            g5h2v2_target => 99.0,
            g5h2v2_strength => 99.0,
            g5h2v2_temperature => 99.0,
        )),
        algorithm;
        backend = CPUBackend(),
        scalar_type = Float32,
    ...
    @test _g5h2v2_same_state(last(continued), last(resumed))
    ...
    other = _g5h2v2_fixture(:g5h2v2_checkpoint_other)
    other_problem = _g5h2v2_problem(
        other;
        tspan = problem.tspan,
        seed = problem.seed,
...
@testset "ensemble replica and repeat identity" begin
    ...
    @test all(
        ...
        all(==(1.25f0), first(solution)[:g5h2v2_marker])
        for solution in (serial.u..., threaded.u...)
    )
    @test all(
        _g5h2v2_same_state(last(left), last(right))
...
@testset "surfaced callback failure" begin
    ...
    mutate_then_save = SciMLBase.DiscreteCallback(
        (_, time, _) -> time == 1,
        callback_integrator -> G5H2V2_SII.setp(
            callback_integrator,
            g5h2v2_target,
    ...
    @test no_op_integrator.capability_report.maturity ===
