function _host_relationship_fixture(name::Symbol)
...
function _active_relationship_edge(topology)
...
@testset "settled host relationship transaction is atomic and generation-safe" begin
    fixture = _host_relationship_fixture(:host_relationship)
    integrator = init(
        fixture.problem,
        SequentialCPM();
        backend = CPUBackend(),
    ...
    relationship_name = Symbol(:host_relationship, :_links)
    ...
    edge = _active_relationship_edge(topology)
    @test edge !== nothing
...
@testset "host relationship mutation rolls back with a failing callback set" begin
    fixture = _host_relationship_fixture(:host_callback)
    relationship_name = Symbol(:host_callback, :_links)
...
@testset "host relationship transaction rebuilds checkerboard banks" begin
    fixture = _host_relationship_fixture(:host_checkerboard)
    relationship_name = Symbol(:host_checkerboard, :_links)
    integrator = init(
        fixture.problem,
