...
@testset "distinct medium owners may share one interaction kind" begin
    medium = MediumKind(:shared_interaction_medium)
    ...
    scheduled = mtkcompile(PottsSystem(
        ...
        statements = StatementSet((
            Lattice((4, 4); boundary = Closed(), default_owner = bulk,
    ...
    initial = PottsInitialState(ownership = OwnershipLayout(
        (4, 4),
        MediumPlacement(island, ((1, 1), (1, 2))),
        CellPlacement(1, cell, ((2, 2),));
        medium = bulk,
    ))
    for algorithm in (SequentialCPM(), CheckerboardSweepCPM())
        integrator = init(PottsProblem(scheduled, initial, (0, 1); seed = 0x0c11),
            algorithm; scalar_type = Float32)
        plan = getfield(integrator, :plan)
        domain = CartesianSPI.cartesian_domain(plan.core_program)
        report = CartesianSPI.cartesian_domain_report(domain)
        @test report.default_owner.kind == only(report.domain_owners).kind
        @test report.default_owner.identity != only(report.domain_owners).identity
        island_code = CartesianSPI.domain_owner_code(domain,
            only(report.domain_owners).identity)
        @test island_code < 0
        @test count(==(island_code), integrator.u.ownership) == 2
        @test count(==(Int32(0)), integrator.u.ownership) == 13
...
@testset "3D mixed Cartesian faces preserve authored order and obstacle set" begin
    medium = MediumKind(:mixed_3d_medium)
        ...
        statements = StatementSet((
            Lattice((4, 4, 2);
                boundary = (
                    ...
                    AxisBoundary(FixedExterior(bulk), Closed()),
                ),
                default_owner = bulk,
                domain_owners = (wall,),
            ...
            cell, medium,
            ProposalConstraint(:frozen_mixed_3d, false),
            Protocol(Sweep(); name = :main),
        )),
    ))
    plan = Potts._lower_scheduled_execution_plan(
    ...
    @test getproperty.(report.faces, :kind) == (
        ...
        CartesianSPI.FixedExteriorCartesianFace,
        CartesianSPI.FixedExteriorCartesianFace,
        CartesianSPI.ClosedCartesianFace,
    )
