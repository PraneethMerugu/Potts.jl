@testset "serial runtime reactive activation and explicit iteration" begin
    scale = TimeScale(1)
    schema = BranchSchema(
    ...
    model = compose(:SerialIterationFixture, schema; scale) do builder, stores
    ...
    @test_throws ModelValidationError compose(
        :SerialCycleFixture, schema; scale) do builder, stores
        trigger = mount!(builder, :trigger, C15Producer())
        schedule!(builder, trigger, Every(Duration(1, scale)))
        attach!(builder, trigger, (out=stores.trigger,))
