@testset "serial runtime typed continuation contracts" begin
    scale = TimeScale(1)
    ...
    bad_initial_model = compose(
        :SerialBadInitialContinuation, state_schema; scale) do builder, stores
        bad = mount!(
    ...
    bad_proposal_model = compose(
        :SerialBadProposalContinuation, state_schema; scale) do builder, stores
        bad = mount!(
...
@testset "serial runtime observer continuation and projection privacy" begin
    scale = TimeScale(1)
    ...
    private_model = compose(
        :SerialPrivateObservation, private_schema; scale) do builder, stores
