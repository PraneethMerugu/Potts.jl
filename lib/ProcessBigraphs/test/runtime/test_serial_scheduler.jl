@testset "serial runtime strict serial scheduler" begin
    scale = TimeScale(1)
    ...
    empty_model = compose(:SerialEmpty, empty_schema; scale) do _, _
    end
...
@testset "serial runtime adaptive schedules and horizon failures" begin
    scale = TimeScale(1)
    schema = BranchSchema(
        state=LeafSchema(Int; default=0, update_law=:add),
    )
    valid_model = compose(:SerialAdaptiveValid, schema; scale) do builder, stores
    ...
    invalid_model = compose(:SerialAdaptiveInvalid, schema; scale) do builder, stores
    ...
    no_partial_model = compose(
        :SerialAdaptiveNoPartial, schema; scale) do builder, stores
...
@testset "serial runtime multirate interval inputs" begin
    scale = TimeScale(1)
    for behavior in (:frozen, :interpolated, :event_updated,
        ...
        model = compose(
            Symbol(:SerialMultirate_, behavior), schema; scale) do builder, stores
