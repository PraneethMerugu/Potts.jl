function serial_failure_composite()
    scale = TimeScale(1)
    schema = BranchSchema(
        state=LeafSchema(Int; default=0, update_law=:add),
        copied=LeafSchema(Int; default=0, update_law=:replace),
    )
    model = compose(:SerialFailureFixture, schema; scale) do builder, stores
...
function serial_runtime_boundary(runtime)
...
@testset "serial runtime transactional failure stages" begin
    scale = TimeScale(1)
    compiled = serial_failure_composite()
    ...
    baseline_boundary = serial_runtime_boundary(baseline)
    ...
    for stage in stages
        ...
        stable = serial_runtime_boundary(runtime)
        error = try
        ...
        @test serial_runtime_boundary(runtime) == stable
        ...
        run_until!(runtime, LogicalTime(1, scale))
        @test serial_runtime_boundary(runtime) == baseline_boundary
    ...
    first_boundary = serial_runtime_boundary(later)
    ...
    @test serial_runtime_boundary(later) == first_boundary
