...
function open_composite_fixture(
...
function paired_open_composite(;
    ...
    components = Dict(
        :left => CompositeMount(:left,
            open_composite_fixture("left_definition", 1; initial=left_initial)),
        :right => CompositeMount(:right,
            open_composite_fixture("right_definition", 2; initial=right_initial)),
    )
...
function contract_open_composite_fixture(
    definition::AbstractString,
    leaf::LeafSchema;
    role::Symbol=:bidirectional,
    transfer=nothing,
...
function composite_payload_keywords(component)
...
const OPEN_COMPOSITE_OBJECT_LAYOUT = (
    Composite=(
...
const OPEN_COMPOSITE_HOM_TARGETS = Dict(
...
function reverse_structure_rows(source)
    target = ProcessBigraphACSet()
    row_maps = Dict{Symbol,Dict{Int,Int}}()
    for (object, _) in pairs(OPEN_COMPOSITE_OBJECT_LAYOUT)
    ...
    for (object, layout) in pairs(OPEN_COMPOSITE_OBJECT_LAYOUT)
        for old in ACSets.parts(source, object)
            ...
            for hom in layout.homs
                ...
                new_target = row_maps[OPEN_COMPOSITE_HOM_TARGETS[hom]][old_target]
...
@testset "open composition same-schema boundaries and Catlab structured cospans" begin
    component = open_composite_fixture("component", 1)
...
@testset "open composition n-ary composition, hierarchy lowering, and runtime" begin
    composed = paired_open_composite()
...
@testset "open composition authoring, order, and grouping invariance" begin
    forward = paired_open_composite()
    reversed = paired_open_composite(
        mount_order=(:right, :left),
        endpoint_order=(:right, :left),
    )
    left_grouped = paired_open_composite(grouping=:left)
    right_grouped = paired_open_composite(grouping=:right)
    ...
    cospan_authored = compile_composite(
        ...
        composite_payload_keywords(forward)...,
    )
    ...
    reversed_structure = reverse_structure_rows(canonical_structure(forward))
...
@testset "open composition annotated directed-wiring round trip" begin
    composed = paired_open_composite()
...
@testset "open composition nested immutable composition" begin
    inner = paired_open_composite()
    third = open_composite_fixture("third_definition", 3)
    nested = compose_open(
        "outer_root";
        mounts=(
    ...
    current = open_composite_fixture("depth_zero", 1)
    for level in 1:5
...
@testset "open composition endpoint roles, privacy, and repeated definitions" begin
    ...
    for left_role in roles, right_role in roles,
        ...
        mounts = (
            CompositeMount(:left,
                open_composite_fixture("same_definition", 1; role=left_role)),
            CompositeMount(:right,
                open_composite_fixture("same_definition", 2; role=right_role)),
        )
...
@testset "open composition n-way junctions and exact endpoint contracts" begin
    components = tuple((
        CompositeMount(Symbol("component_$(index)"),
            open_composite_fixture("shared_definition", index))
        for index in 1:4
    )...)
    references = tuple((
    ...
    left = contract_open_composite_fixture("contract_left", baseline)
    for (index, different) in enumerate(incompatible)
        right = contract_open_composite_fixture("contract_right_$(index)", different)
    ...
    transferred = contract_open_composite_fixture(
        "transferred",
        baseline;
        transfer,
    )
    ...
    transferred_peer = contract_open_composite_fixture(
        "transferred_peer",
        baseline;
        transfer,
    )
...
@testset "open composition initialization and composition failures are atomic" begin
    left = open_composite_fixture("left_definition", 1; initial=1)
    right = open_composite_fixture("right_definition", 2; initial=2)
    ...
    unresolved = contract_open_composite_fixture(
    ...
    imports = (
        CompositeMount(:left,
            open_composite_fixture("import_left", 1; role=:import)),
        CompositeMount(:right,
            open_composite_fixture("import_right", 2; role=:import)),
    )
    @test_throws ProcessBigraphError compose_open(
        "no_provider";
        mounts=imports,
    ...
    @test_throws ProcessBigraphError compile_composite(
        malformed;
        composite_payload_keywords(left)...,
    )
end
