...
function canonical_structure_fixture(order=("fast", "slow"))
    scale = TimeScale(1)
    schema = BranchSchema(
...
function canonical_structure_payloads(composite)
...
@testset "canonical structure canonical ProcessBigraph ACSet" begin
    composite = canonical_structure_fixture()
...
@testset "canonical structure row, declaration, and authoring-path invariance" begin
    forward = canonical_structure_fixture(("fast", "slow"))
    reversed = canonical_structure_fixture(("slow", "fast"))
    ...
    laws, continuations = canonical_structure_payloads(forward)
...
@testset "canonical structure frozen epoch and fail-closed corruption" begin
    composite = canonical_structure_fixture()
    model = canonical_model(composite)
    structure = canonical_structure(model)
    laws, continuations = canonical_structure_payloads(composite)
