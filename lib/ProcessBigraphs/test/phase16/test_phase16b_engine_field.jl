...
struct P16BMockAdapter <: AbstractEngineAdapter
    mode::Symbol
end

engine_semantic_parameters(adapter::P16BMockAdapter) = (mode=adapter.mode,)

mutable struct P16BMockInstance <: AbstractEngineInstance
    mode::Symbol
...
prepare_engine(adapter::P16BMockAdapter, declaration::EngineDeclaration) =
    P16BMockInstance(adapter.mode, [0.0], nothing, 0)

struct P16BThrowCompletionHandle <: AbstractCompletionHandle end

complete_operation!(
    instance::P16BMockInstance,
    ::P16BThrowCompletionHandle,
...
function stage_operation!(
    ...
    elseif instance.mode === :complete_throw
        return P16BThrowCompletionHandle()
...
function p16b_publication_allocations(
    instance::P16BMockInstance,
    invocation::EngineInvocation,
    candidate::EngineCandidate,
)
...
function p16b_declaration(mode::Symbol=:success)
    capabilities = EngineCapabilities(
        operation_families=(:interval_advance, :boundary_solve, :discrete_batch),
        problem_envelopes=("phase16b-mock",),
        backends=(:cpu,),
        precisions=(:float64,),
        residencies=(:host,),
    ...
    EngineDeclaration("phase16b-mock", P16BMockAdapter(mode); capabilities)
end

function p16b_invocation(
...
@testset "Phase 16.B solver-neutral engine transaction" begin
    ...
    for (mode, code) in (
        ...
        staged_declaration = p16b_declaration(mode)
        staged_instance = prepare_engine(staged_declaration)
        staged_error = try
            execute_engine!(
                staged_instance,
                p16b_invocation(staged_declaration, [1.0]),
            )
            nothing
        catch caught
            caught
    ...
    discard_declaration = p16b_declaration(:discard_throw)
    discard_instance = prepare_engine(discard_declaration)
    discard_error = try
        execute_engine!(
            discard_instance,
            p16b_invocation(discard_declaration, [1.0]);
    ...
    early_declaration = p16b_declaration(:early)
    early_instance = prepare_engine(early_declaration)
    early = execute_engine!(
        early_instance,
        p16b_invocation(early_declaration, [1.0]),
    )
    ...
    event_declaration = p16b_declaration(:event)
    event = execute_engine!(
        prepare_engine(event_declaration),
        p16b_invocation(event_declaration, [1.0]),
    )
...
@testset "Phase 16.B typed continuation and restart cuts" begin
    continuation = EngineContinuation(
        "engine-1",
        "state-v1",
...
@testset "Phase 16.B field descriptor, sampling, deposition, and exchange" begin
    scale = TimeScale(1)
...
@testset "Phase 16.B exact field clocks and named splitting" begin
    scale = TimeScale(1)
