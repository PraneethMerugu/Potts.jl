...
function serial_open_component(definition, amount)
    scale = TimeScale(1)
    schema = BranchSchema(
...
function serial_open_fork_join(; grouping=:nary)
    components = (
        CompositeMount(:left, serial_open_component("left", 1)),
        CompositeMount(:middle, serial_open_component("middle", 2)),
        CompositeMount(:right, serial_open_component("right", 3)),
    )
...
@testset "serial runtime six-path authoring equivalence" begin
    nary = serial_open_fork_join(grouping=:nary)
    grouped = serial_open_fork_join(grouping=:left)
    right_grouped = serial_open_fork_join(grouping=:right)
