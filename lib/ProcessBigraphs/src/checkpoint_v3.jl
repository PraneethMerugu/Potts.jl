const PHASE16_CHECKPOINT_VERSION = "3.0.0"
const PHASE16_CHECKPOINT_SCHEMA = "process-bigraph-logical-checkpoint-v3"
...
struct RestoredPhase16Checkpoint{R,S,E}
    runtime::R
    structural_epoch::S
    engines::E
    replay_class::Symbol
...
function _restore_structural_topology(payload)
    ...
    tuple((value.object for value in payload)...) == expected_objects ||
        _fail(:checkpoint_topology_schema_mismatch,
            "Phase 16 checkpoint topology object order is incompatible")
        ...
        for row in object_payload.rows
            keys(row) == expected_properties ||
                _fail(:checkpoint_topology_row_mismatch,
                    "Phase 16 checkpoint topology row has incompatible fields";
...
function phase16_checkpoint(
    ...
    runtime.is_settled ||
        _fail(:unsettled_checkpoint,
            "Phase 16 checkpoints require a settled serial runtime")
    ...
    length(owners) == length(unique(owners)) ||
        _fail(:duplicate_checkpoint_component,
            "Phase 16 checkpoint contains duplicate component owners")
    ...
    payload = (
        schema=PHASE16_CHECKPOINT_SCHEMA,
        format_version=PHASE16_CHECKPOINT_VERSION,
...
function _validate_phase16_checkpoint(checkpoint::LogicalCheckpointV3)
    checkpoint.format_version == PHASE16_CHECKPOINT_VERSION ||
        _fail(:unsupported_checkpoint_version,
            "Phase 16 checkpoint format is unsupported";
    ...
    payload_bytes == checkpoint.payload_bytes ||
        _fail(:checkpoint_payload_mismatch,
            "Phase 16 checkpoint cached bytes differ from its payload")
    integrity = bytes2hex(sha256(payload_bytes))
    integrity == checkpoint.integrity ||
        _fail(:checkpoint_integrity_failure,
            "Phase 16 checkpoint integrity hash does not match";
            expected=checkpoint.integrity, actual=integrity)
    checkpoint.payload.schema == PHASE16_CHECKPOINT_SCHEMA &&
        checkpoint.payload.format_version == PHASE16_CHECKPOINT_VERSION ||
        _fail(:unsupported_checkpoint_schema,
            "Phase 16 checkpoint payload schema is unsupported")
    ...
    aggregate_replay_class(Symbol[
        ...
        _fail(:checkpoint_replay_mismatch,
            "Phase 16 aggregate replay classification is invalid")
    true
end

function encode_checkpoint(checkpoint::LogicalCheckpointV3)
    _validate_phase16_checkpoint(checkpoint)
...
function decode_phase16_checkpoint(bytes::AbstractVector{UInt8})
    ...
    envelope isa NamedTuple &&
        ...
        _fail(:invalid_checkpoint_envelope,
            "decoded Phase 16 checkpoint has an invalid envelope")
    envelope.schema == PHASE16_CHECKPOINT_SCHEMA ||
        _fail(:unsupported_checkpoint_schema,
            "checkpoint is not a Phase 16 logical envelope";
            schema=envelope.schema)
    payload_bytes = encode_logical_value(envelope.payload)
    checkpoint = LogicalCheckpointV3(
...
checkpoint_fingerprint(checkpoint::LogicalCheckpointV3) =
...
function _validate_structural_restore(
    ...
    payload.contract_version == STRUCTURAL_TRANSACTION_VERSION ||
        _fail(:checkpoint_structure_version_mismatch,
            "Phase 16 checkpoint structural contract is incompatible";
    ...
    payload.topology_fingerprint == structural_fingerprint(structure) ||
        _fail(:checkpoint_structure_mismatch,
            "Phase 16 checkpoint structural topology fingerprint is invalid")
    ...
    expected == restored.fingerprint ||
        _fail(:checkpoint_structure_fingerprint_mismatch,
            "Phase 16 checkpoint structural epoch fingerprint is invalid";
...
function restore_phase16_checkpoint(
