...
The architecture has one canonical ACSet-to-compiled-plan boundary, immutable open composition,
an immutable-topology serial executor, semantic RNG, typed observation and continuation,
transactional failure, and portable logical checkpoints. Atomic dynamic structural transactions,
solver-neutral engine adapters, native and SciML field paths, bounded Merks and CNV assemblies,
and semantic high-level authoring build on those layers.

Distributed execution, Dagger equivalence, universal solver support, full publication analyses,
and public release remain outside the internal-beta boundary. `AlgebraicDynamics.jl` is not a
current dependency or qualified extension.

Exact-head qualification requires Linux x86_64, macOS ARM64, independent-package, integration,
rendering, oracle, documentation, applicable hardware, and frozen performance gates.
Historical qualification artifacts and their original names are isolated in the repository
...
Open composition extends the same ACSet with composite containment, typed endpoints, boundary maps,
...
Dynamic add/remove/divide/move/rewire operations are qualified coupled runtime structural transactions.
...
no foundation invocation receives mutable committed storage.
...
The runtime implements version-1 forms of additive, multiplicative, unique replace,
...
The runtime records transfer declarations and their bounds but does not execute or measure a device
transfer. native-field qualification separately qualifies the native Cartesian field envelope on real Metal and
...
`SerialExecutor()` is the fail-closed serial runtime policy and `SerialRuntime` owns its mutable
execution state. The one-argument `initialize_runtime(compiled)` remains a legacy-compatibility
facade solely to preserve foundation/15.A/15.B fingerprints; new qualified work constructs the executor
explicitly.
...
identities, topology, continuation codecs, observer positions, and payload integrity. coupled runtime
...
[failure and persistence guide](failure-and-persistence.md) for the full coupled runtime contract.
