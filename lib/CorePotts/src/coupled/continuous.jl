...
mutable struct EvolvingFieldState{A <: AbstractArray, T, B, W}
    name::Symbol
    values::A
    forcing::A
    spacing::T
...
function EvolvingFieldState(name::Symbol, values::AbstractArray{T};
    ...
    fill!(first, zero(T))
    fill!(second, zero(T))
    workspace = FieldAdvanceWorkspace(first, second)
    ...
    return EvolvingFieldState(name, authoritative, forcing,
        resolved_spacing, boundary, resolved_time, diagnostics, workspace)
end
...
function _field_values_valid(values)
    for value in values
...
function _advance_transient_field!(state::EvolvingFieldState,
    ...
    for step in 1:count
        ...
        _field_values_valid(output) || throw(ArgumentError(
