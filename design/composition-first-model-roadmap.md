...
| G05 / R10–R11 | Defend neighborhood geometric means/zero behavior for activity, geometry-dependent adhesion, substrate-restricted moments and sensing, source-aware matrix updates, and multiple consumers of shared quantities. Keep existing maintained-minimum and numerical obligations. Only add missing reusable mathematics in its true owner. |
| G05C / R49–R50 | Keep the accepted pair and downstream canaries. Add actual model-derived cases to the existing criteria: geometry-dependent contact energy and field/history consumption. R49 establishes canonical Core executor identity across author-only renames, numerical value changes and bounded entry-count ladders; R50 removes author identity/irregular graph structure before that boundary and proves package-declared and interactive compositions share it. No third compiler rewrite, model tag, retained builder graph or parallel dependency authority. |
| G06 / R12–R13 | Demonstrate ordinary activity and chemotaxis drives, anisotropic contact energy and coupled mechanical deltas. Conservative energy tests use full tiny-system differences; active drive tests use its stated transition bias. Do not demand a global Hamiltonian for a nonequilibrium drive. |
| G07 / R14–R16 | Complete native expression/structured IO, reciprocal field feedback, native unknown/observed initialization, explicit physical-time mapping, solver/lifecycle ownership and failure semantics. Keep upstream MTK systems native. Include a maintained MethodOfLines path and selected SBML-imported native systems. R16 remains a small reusable coupling witness, not a substitute for #6 or #11. |
...
| E03 / R26–R27 | Test ordered multi-segment bending through actual public traversal/relations and inverse dependencies. A pair-endpoint contract alone is insufficient. Extend the existing contract only if coherent; otherwise count the demonstrated owner companion(s). |
...
| E08 / R36–R38 | Keep exact topology and sampled morphology distinct. R38 completes the invasion-analysis witness #10 using the actual published measures. Do not impose topology constraints merely to stabilize angiogenesis or compartment pictures. Any morphological algorithm beyond the selected invariant must have an identified owner and test. |
...
| E11–E12 / R45–R48 | Preserve swap and static weighted-graph commitments. None of these fourteen papers automatically witnesses them: add explicitly labeled feature combinations, not altered paper reproductions. |
