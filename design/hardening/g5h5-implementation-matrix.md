...
|:--|:--|:--|:--|:--|
| H5-A | Executable final-interface documentation for every stable authoring and execution concept | Curated `docs/make.jl` page tree and rewritten `docs/src` manual | Ten learn/concept pages, two published-model pages, and three API pages are the exact `docs/make.jl` source set; doctests, cross-references, checks, and HTML rendering passed with `warnonly=false` | passed |
